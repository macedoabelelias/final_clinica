...
<table class="table table-striped borda" cellpadding="6">
  ...
  <tbody>
	...
	$query2 = $pdo->query("SELECT * FROM valor_parcial WHERE id_conta = '$id_conta' and tipo = 'Receber'");
	...
	//totalizar contas despesas
  	...
  	$query = $pdo->query("SELECT * FROM pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Conta'  ORDER BY data_pgto asc");
	...
	$query2 = $pdo->query("SELECT * FROM valor_parcial WHERE id_conta = '$id_conta' and tipo = 'Pagar'");
	...
	if(@count($res2) > 0){		
		...
		for($i2=0; $i2 < @count($res2); $i2++){
			...
			$total_resid_pagar += $valor_resid;			

		}

		

		$total_pagar += $total_resid_pagar;
